...
1. Apply migrations through `2026082806_overlay_device_key_history.up.sql` before deploying the service. The key-history migration deliberately fails if an interim deployment reused a revoked key; investigate the conflicting identities instead of deleting a row or choosing a winner.
...
Rollback is guarded. First return every node to shadow and ensure there are no apply-mode node-status/apply-result rows. The down migration drops `overlay_device_key_history` and therefore destroys permanent key-reuse protection; stop all device writes, take an encrypted backup, and require explicit security approval before running it. A normal application rollback should keep the Batch 07 schema and tombstones in place. Database backups must remain encrypted and access to device public keys/events limited to the Accounts runtime role.
