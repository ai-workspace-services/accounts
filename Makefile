...
CLOUD_RUN_ENV ?= prod
CLOUD_RUN_SERVICE ?= $(if $(filter preview,$(CLOUD_RUN_ENV)),preview-accounts-svc-plus,accounts-svc-plus)
...
CLOUD_RUN_IMAGE ?= $(GCP_REGION)-docker.pkg.dev/$(GCP_PROJECT)/cloud-run-source-deploy/accounts.svc.plus/$(CLOUD_RUN_SERVICE):latest
...
export APP_NAME MAIN_FILE PORT OS \
	...
	GCP_PROJECT GCP_REGION CLOUD_RUN_ENV CLOUD_RUN_SERVICE CLOUD_RUN_SERVICE_YAML CLOUD_RUN_STUNNEL_CONF CLOUD_RUN_IMAGE \
